...
export const BurgerConstructor: FC = () => {
  ...
  const { ingredients, bun } = useSelector((state: any) => state.burgerConstructor)
  let total = 0
  if (bun !== undefined && ingredients.length > 0) {
    ingredients?.map((elem: TIngredientData) => total += elem.price)
    total +=  bun.price * 2
  } else {
    total = 0
  }
  ...
  function addIngredientInConstructor(ingredient: TIngredientData) {
  ...
  function deleteIngredientFromConstructor(ingredient: TIngredientData) {
      dispatch({
        type: DECREASE_COUNT,
        ingredient: ingredient,
        _id: ingredient._id,
  ...
  const handlePostOrder = async () => {
    const dataRequest = { ingredients: [ bun._id , ...ingredients.map((ingredient: TIngredientData) => ingredient._id) , bun._id ]}
    if (isAuth) {
      dispatch<any>(postOrder(dataRequest))
    } else {
      navigate(PATH.LOGIN)
    }
  }
      ...
      <div className={`${style.containerConstructor}`}>
        <div className={`${style.containerConstructorInside}`}>
          {(ingredients.length === 0) ? (<span className={`${style.startSectionIngredient} + text text_color_primary`}> Add ingredient in your burger </span>) :
            ingredients?.map((elem: TIngredientData, index: number) =>
            <ConstructorCard
              elem={elem}
              index={index}
              key={nanoid()}
